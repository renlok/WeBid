...
#// Retrieve data
$query = "SELECT year FROM " . $DBPrefix . "currentplatforms WHERE (year<>".date("Y")."
			  OR (year=".date("Y")." AND month<>".date("m").")) GROUP BY year ORDER BY year desc";
$res = mysql_query($query);
$system->check_mysql($res, $query, __LINE__, __FILE__);
while ($year = mysql_fetch_array($res))
{
	$query = "SELECT * FROM " . $DBPrefix . "currentplatforms WHERE year=".$year[year]." AND month<>'".date("m")."' GROUP BY month ORDER BY month desc";
  ...
  <tr> 
	<td background="images/bac_barint.gif"><table width="100%" border="0" cellspacing="5" cellpadding="0">
		<tr> 
		  ...
		  <td class=white><?php echo $MSG['25_0023']; ?>&nbsp;&gt;&gt;&nbsp;<?php echo $MSG['5318']; ?></td>
		</tr>
	  </table></td>
  </tr>
  <tr>
	  ...
	  <td width="626">
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
		  <tr>
			<td width="600"> 
			  <?php
			  $query = "SELECT platform,counter
				   			 FROM " . $DBPrefix . "currentplatforms
